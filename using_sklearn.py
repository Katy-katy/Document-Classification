...
def work(test_data):
    ...
    clf2 = RandomForestClassifier().fit(X_train, labels) #87 points
    
    eclf = VotingClassifier(estimators=[('kn', clf), ('rf', clf2)], voting='soft')# 91.1 point
    eclf.fit(X_train, labels)
    
    X_new_counts = my_vector.transform(test_data)
    X_new_tfidf = tf_transformer.transform(X_new_counts)
    X_new = svd.transform(X_new_tfidf)
    return eclf.predict(X_new)
